...
- `biseccion/`, `punto_fijo/`, `newton_raphson/`, `secante/`, `gauss_seidel/`, `descomposicion_lu/` and `regresion_lineal/` are each their own `src/<package>/` layout with their own `pyproject.toml`, depending on `metodos-numericos-base` as a workspace dependency (`[tool.uv.sources] metodos-numericos-base = { workspace = true }`). Each one's `gui.py` declares a module-level `DESCRIPTOR` (a `DescriptorDeMetodo`) plus an `ejecutar_<metodo>(valores, reportar_iteracion)` adapter that calls the method's own solver — **none of these seven `gui.py` files import `tkinter`**; they only describe their method's inputs/outputs declaratively.
- `metodos_numericos_gui/` is the single app that ties everything together: it's the only package that imports all seven methods' `DESCRIPTOR`s (in `registro.py`), and it's the only place `VentanaMetodosNumericos` gets instantiated. See "Architecture: metodos_numericos_gui" below — that section has the exact recipe for adding a new method.
...
- Always run with the newest resolvable versions of every dependency. Keep dependency specifiers as lower bounds only (`>=`), never pin exact versions or add upper bounds (the one exception is `[build-system] requires = ["uv_build>=…,<…"]`, which is uv's own generated template — leave it), and periodically re-run `uv lock --upgrade` (then `uv sync`) from the repo root to pull the shared lockfile forward.
...
uv run descomposicion-lu                     # descomposicion_lu: hardcoded example
uv run python -c "from descomposicion_lu import resolver_desde_terminal; resolver_desde_terminal()"  # descomposicion_lu: interactive terminal solver

uv run regresion-lineal                      # regresion_lineal: hardcoded example
uv run python -c "from regresion_lineal import resolver_desde_terminal; resolver_desde_terminal()"  # regresion_lineal: interactive terminal solver
...
There is no longer a per-method `<metodo>-gui` script (`biseccion-gui`, `punto-fijo-gui`, `newton-raphson-gui` were removed) — `uv run metodos-numericos` is the only GUI entry point, covering all seven methods.
...
- `terminal.py` — `leer_funcion_desde_terminal(*, etiqueta="f(x)", entrada=input)` reads and validates an expression from the terminal (retrying on a bad one), with a customizable prompt label so a method can ask for `"g(x)"` or `"f'(x)"` instead. `leer_valor_inicial_desde_terminal(*, etiqueta="Valor inicial", entrada=input)` reads a single float, retrying on a bad one. `leer_opcion_desde_terminal` (pick one of a tuple of options) and `leer_puntos_desde_terminal` (a list of (x, y) points) exist for `regresion_lineal`.
- `descriptor.py` — the declarative contract a method exposes to the GUI, with **no tkinter/matplotlib import**: `TipoDeCampo` (`EXPRESION_MATEMATICA`, `NUMERO`, `TEXTO_MULTILINEA`, `CASILLA_DE_VERIFICACION`, `SISTEMA_DE_ECUACIONES`, `TABLA_DE_PUNTOS`, `SELECTOR`), `CampoDeEntrada(nombre, etiqueta, tipo, valor_por_defecto, cantidad_de_lineas, opciones)` (labels use the course's notation: `"a ="`, `"x₀ ="`; `opciones` is the choice list for a `SELECTOR`); `ColumnaDeTabla(clave, encabezado, formato, es_raiz, es_error)` + `FormatoDeColumna` (`NUMERO`/`SIGNO`/`ENTERO`) + `COLUMNA_DE_NUMERO_DE_FILA` — `clave` is `"numero"`/`"aproximacion"`/`"error"` or a `detalle` key, `es_raiz` paints the last row's cell amber, `es_error` paints the first cell below ε green; `desplazamiento_en_filas` shifts a column's cells down that fraction of a row and draws each non-empty one as a loose box (the staircase divided-differences table); `TipoDeGrafico` (`INTERVALOS`, `TELARANA`, `TANGENTES`, `SECANTES`, `CONVERGENCIA`, `DISPERSION_Y_AJUSTE`) + `GraficoDeMetodo(tipo, campo_de_la_funcion)`; `ResultadoDeMetodo(etiqueta_del_valor, valor, cantidad_de_iteraciones, verificaciones, advertencias, matrices, etiquetas_de_componentes, funcion_para_grafico, puntos_de_datos)` — everything raw (floats), the GUI formats; the last three exist for regression (names for each component of a tuple `valor`, the fitted y = f(x), and the scatter points); `Verificacion(etiqueta, valor, esperado)`; `MatrizDeResultado(nombre, filas, es_resultado)`; and `DescriptorDeMetodo(nombre_para_mostrar, capitulo, formula, descripcion, campos, ejecutar, columnas, grafico, usa_criterio_de_parada)`. `columnas` is a callable `valores -> columns` (so Gauss-Seidel can have one Xⱼ/|Eⱼ| pair per unknown; use `columnas_fijas(...)` otherwise) or `None` for no table. When `usa_criterio_de_parada` is true the GUI adds its own ε / "Máx. iteraciones" group and passes `valores["tolerancia"]` (float) and `valores["maximo_iteraciones"]` (int) — every iterative adapter must forward them to its solver. `errores.py`'s `NoConvergeError.despues_de(maximo, ultimo_error)` builds the standard message; `EntradaInvalidaError(mensaje, nombre_del_campo)` lets the GUI mark the offending field.
- `sistemas.py` — `leer_sistema_desde_texto(texto)` parses a linear system written one equation per line (`"12 -1 3 | 8"`, `|`/`=` optional) into `(filas, terminos_independientes)`, raising a `ValueError` naming the offending row. Not specific to any method (`gauss_seidel` and `descomposicion_lu` both use a `SISTEMA_DE_ECUACIONES` field) — `GrillaDeSistema` uses it once, to seed its input grid from a `CampoDeEntrada.valor_por_defecto` string and infer the grid's initial size.
- `puntos.py` — `leer_puntos_desde_texto(texto)` parses an (x, y) table, one point per line (`"2 4.5"`), into `(valores_x, valores_y)`; `GrillaDeDatos` uses it the same way to seed a `TABLA_DE_PUNTOS` field.
- `gui/` — the Tkinter toolkit (a package; same import path `metodos_numericos_base.gui`), visual identity "apunte de cátedra": colors from the handouts (f(x) red, g(x) green, y = x blue, **amber only for the answer** — root cell, result band, root point — and light green for "|E| < ε" cells), Bahnschrift for UI text and Cambria for everything mathematical (numbers, formulas, fields; its digits are tabular, Bahnschrift's are not). Never use Cambria Math in widgets (its linespace is ~5× normal). The base package's `__init__` loads the GUI **lazily** (`__getattr__`), so terminal scripts never import tkinter or matplotlib.
  ...
  - `campos.py` — `TecladoMatematico` (labels in Spanish notation — `sen`, `tg` — insert the `math` names; tracks the target entry via `<FocusIn>`; keys have `takefocus=False`) and `GrillaDeSistema` (extended matrix [A | b] with b in red, "Ecuaciones" spinbox up to `TAMANO_MAXIMO_SISTEMA = 5`, resizing keeps overlapping values) and `GrillaDeDatos` (the Xᵢ/Yᵢ grid behind `TABLA_DE_PUNTOS`).
...
`metodos_numericos_gui/src/metodos_numericos_gui/` is the app: the only package in the workspace that knows about all seven methods.
...
## Architecture: descomposicion_lu

`descomposicion_lu/src/descomposicion_lu/__init__.py` implements LU decomposition (Doolittle, as it falls out of Gaussian elimination, **no pivoting**) to solve `Ax = b`, plus A⁻¹ and the condition number. Depends on `numpy`. A direct method, not iterative.

- `descomponer_lu(matriz_coeficientes, reportar_iteracion=None)` → `(L, U)`; raises `ValueError` on a non-square matrix or a zero pivot. `sustitucion_adelante` / `sustitucion_atras` / `resolver_con_lu` do the two triangular solves. `calcular_inversa` reuses one LU to solve `Ax = eᵢ` per column. `norma_renglon_suma` / `numero_de_condicion` compute Cond[A] = ‖A‖·‖A⁻¹‖ with the row-sum norm.
- `descomposicion_lu(matriz_coeficientes, terminos_independientes, reportar_iteracion=None)` → `(x, inversa, condicion, iteraciones)` — the entry point that chains all of the above.
- `leer_matriz_desde_terminal` / `resolver_desde_terminal` / `main()` — same shape as `gauss_seidel`'s terminal layer; `main()` runs the textbook examples 10.1–10.3.
- `gui.py` — no `tkinter` import. `DESCRIPTOR` has one `SISTEMA_DE_ECUACIONES` field (linked to Gauss-Seidel's grid), `usa_criterio_de_parada=False`, and **no `columnas`/`grafico`**, so the panel shows `VistaDeMatrices` with L, U, y, x (the `es_resultado` one) and A⁻¹. `ejecutar_descomposicion_lu` adds a Cond[A] `Verificacion`, one per equation, and a warning when Cond[A] > `UMBRAL_MAL_CONDICIONADO` (100, an arbitrary threshold).

## Architecture: regresion_lineal

`regresion_lineal/src/regresion_lineal/__init__.py` implements least-squares linear regression, with linearization for non-linear models. `MODELOS_DISPONIBLES = ("Lineal", "Exponencial", "Potencial", "Crecimiento")`: exponential fits (x, Ln y), power fits (Log x, Log y) (base 10), growth fits (1/x, 1/y). No `numpy`.

- `ajuste_lineal(valores_u, valores_v)` → `(a₀, a₁)`, the plain least-squares line; raises `ValueError` on mismatched lengths, fewer than 2 points, or all uᵢ equal.
- `regresion_lineal(tipo_modelo, valores_x, valores_y, reportar_iteracion=None)` → `(parametros, funcion)`: `parametros` always has `"a0"`/`"a1"` (in the linearized space) plus `"A"` and `"B"` (`"b"` for Crecimiento) for non-linear models; `funcion` is y = f(x) back in the original space. Raises `ValueError` when the data violate the model's domain (e.g. yᵢ ≤ 0 for Exponencial). Reports one `Iteracion` per **point** (not per iteration), `error=None`, with `detalle` keys `valor_x`, `valor_y`, `u`, `v`, `u_cuadrado`, `uv` — the handout's sums table.
- `resolver_desde_terminal` asks for the model (`leer_opcion_desde_terminal`) and the points (`leer_puntos_desde_terminal`); `main()` fits guide exercise 1.1 (y = 0,8393x + 0,0714).
- `gui.py` — no `tkinter` import. `DESCRIPTOR` has a `SELECTOR` field `modelo` and a `TABLA_DE_PUNTOS` field `puntos`, `usa_criterio_de_parada=False`, `armar_columnas(valores)` picking the table's columns per model, and `GraficoDeMetodo(TipoDeGrafico.DISPERSION_Y_AJUSTE)` fed by `ResultadoDeMetodo.funcion_para_grafico`/`puntos_de_datos`.

## Architecture: interpolacion_newton

`interpolacion_newton/src/interpolacion_newton/__init__.py` implements Newton's divided-differences interpolating polynomial. No `numpy`.

- `interpolacion_newton(valores_x, valores_y, reportar_iteracion=None)` → `(coeficientes, polinomio)`: `coeficientes` are b₀…bₙ (the table's first row, bₖ = f[xₖ;…;x₀]), `polinomio` evaluates Pₙ(x) in nested form. Raises `ValueError` on mismatched lengths, fewer than 2 points, or repeated xᵢ. Reports one `Iteracion` per point (rows 0..n) with `detalle` keys `valor_x`, `valor_y`, `orden_k` = f[xᵢ₊ₖ;…;xᵢ] (row i has n − i of them).
- `main()` runs Chapra's example 18.2 (ln 2 from 4 points, P₃(2) = 0,6288).
- `gui.py` — no `tkinter` import. `TABLA_DE_PUNTOS` field `puntos` plus a `NUMERO` field `valor_a_interpolar` (shown as a P(x) `Verificacion`); `armar_columnas` gives order k `desplazamiento_en_filas=k/2`, which reproduces the staircase table from the whiteboard; `DISPERSION_Y_AJUSTE` plot.
